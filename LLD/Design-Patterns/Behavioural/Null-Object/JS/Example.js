...
function printUser(id){
    ...
    /*
        We need to explicitly tell the console.log to print  `Guest` if 
        the user dows not have a name.

        This is problematic since we need to remember to always put 
        this every we use the users name.

        It is also bad because if we want to print `Unkown User`
        instead, we would need to change every place that we put `Guest`
